...
/* 
  Using apache tika to process pdfs.
  We use this library to get the file type from the file metadata. We need to know if the file really is pdf, or only the name says so.
  We need this library to get the iban information out of the pdf, in case it is a pdf.
*/
...
public class PdfParser {
    ...
    public static String extractContentUsingParser(InputStream stream) throws IOException, TikaException, SAXException {
        ...
        ContentHandler handler = new BodyContentHandler(40000000);
        Metadata metadata = new Metadata();
    ...
    public static String extractContent(InputStream stream) throws IOException, TikaException, SAXException {
        Parser parser = new AutoDetectParser();
        ParseContext context = new ParseContext();
        BodyContentHandler handler = new BodyContentHandler(40000000);
        Metadata metadata = new Metadata();
        parser.parse(stream, handler, metadata, context);
        return handler.toString();
    }
